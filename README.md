...
3. **AI interpretation** (`ai/insights.service.ts`) — takes the Level 1/2
   output, sends it to Anthropic's API through a structured tool-call
...
environment-driven. AI features (`ANTHROPIC_API_KEY`) and Google sign-in
...
For a production target, point `WEB_ORIGIN` (server) and
`NEXT_PUBLIC_API_URL` (web build arg) at your real domains, and run
`prisma migrate deploy` (already wired into the server's Docker `CMD`).
Any standard container host (Fly.io, Railway, Render, a VPS) works — there
is no dependency on a specific platform's proprietary services.
